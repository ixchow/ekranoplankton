...
SHADERS.load = function SHADERS_load(gl) {
	...
	`);
	SHADERS.texture = makeProgram(gl,`
	...
	`,`
		...
		void main() {
			gl_FragColor = texture2D(uTex, vTexCoord.xy) * vec4(1,1,1,vTexCoord.z);
		}
	`);

	SHADERS.solid = makeProgram(gl,`
		attribute vec4 aPosition;
